    ...
    <div class="container">
        <el-card class="box-card">
            <div slot="header" class="clearfix">
                <span> App广告 </span>
                <router-link class="pull-right el-button el-button--primary el-button--small" :to="{ path:'/appAdvert/create'}">添加</router-link>
            </div>
            ...
            <el-table class="table-list" :data="list" border @selection-change="selection">
                ...
                <el-table-column label="投放时间" width="220">
                    <template scope="props">
                        {{ props.row.ad_begin * 1000 | date }}~{{ props.row.ad_end * 1000 | date }} <br> {{ props.row.ad_pay_time | timeSplit }}
                    </template>
                </el-table-column>
                ...
                <el-table-column width="320" class="text-right">
                    <template scope="scope">
                        <span v-if="scope.row.ad_end > now && scope.row.ad_begin < now">
...
export default {
    ...
    methods: {
        ...
        exportTable(){
            return this.exportAppAdvertData()
        },
        selection(checkeds) {
            this.checkeds = checkeds
        },
        ...mapActions(['getAppAdvertList', 'deleteAppAdvert', 'batchDeleteAppAdvert', 'editAppAdvert','exportAppAdvertData'])
    },
